...
int FlashRequestreadPage_cb (  struct PortalInternal *p, const uint32_t card, const uint32_t bus, const uint32_t chip, const uint32_t block, const uint32_t page, const uint32_t tag ) {
    (static_cast<FlashRequestWrapper *>(p->parent))->readPage ( card, bus, chip, block, page, tag);
    return 0;
};
int FlashRequestwritePage_cb (  struct PortalInternal *p, const uint32_t card, const uint32_t bus, const uint32_t chip, const uint32_t block, const uint32_t page, const uint32_t tag ) {
    (static_cast<FlashRequestWrapper *>(p->parent))->writePage ( card, bus, chip, block, page, tag);
    return 0;
};
int FlashRequesteraseBlock_cb (  struct PortalInternal *p, const uint32_t card, const uint32_t bus, const uint32_t chip, const uint32_t block, const uint32_t tag ) {
    (static_cast<FlashRequestWrapper *>(p->parent))->eraseBlock ( card, bus, chip, block, tag);
    return 0;
};
...
int FlashRequestdebugDumpReq_cb (  struct PortalInternal *p, const uint32_t card ) {
    (static_cast<FlashRequestWrapper *>(p->parent))->debugDumpReq ( card);
    return 0;
};
...
int FlashIndicationreadDone_cb (  struct PortalInternal *p, const uint32_t tag, const uint64_t cycles ) {
    (static_cast<FlashIndicationWrapper *>(p->parent))->readDone ( tag, cycles);
    return 0;
};
int FlashIndicationwriteDone_cb (  struct PortalInternal *p, const uint32_t tag, const uint64_t cycles ) {
    (static_cast<FlashIndicationWrapper *>(p->parent))->writeDone ( tag, cycles);
    return 0;
};
int FlashIndicationeraseDone_cb (  struct PortalInternal *p, const uint32_t tag, const uint32_t status, const uint64_t cycles ) {
    (static_cast<FlashIndicationWrapper *>(p->parent))->eraseDone ( tag, status, cycles);
    return 0;
};
int FlashIndicationdebugDumpResp_cb (  struct PortalInternal *p, const uint32_t card, const uint32_t debug0, const uint32_t debug1, const uint32_t debug2, const uint32_t debug3, const uint32_t debug4, const uint32_t debug5 ) {
    (static_cast<FlashIndicationWrapper *>(p->parent))->debugDumpResp ( card, debug0, debug1, debug2, debug3, debug4, debug5);
    return 0;
};
